...
pub fn multibody_selection_system(
    ...
    non_multibodies: Query<(Entity, &Select<GroupDynamic>), (Without<MultibodyRoot>, Without<MultibodyChild>)>
) {
    ...
    for event in interaction_event_reader.iter() {
        ...
        let (links, root_entity) = if let Ok((root_entity, multi_root, _)) = multibody_roots.get(*entity) {
