...
use kesko_physics::{
    ...
    joint::{
        prismatic::PrismaticJoint, revolute::RevoluteJoint, spherical::SphericalJoint,
        JointMotorEvent, JointType, KeskoAxis, MotorCommand,
    },
...
impl MultibodyUIComponent {
    ...
    fn parse_events(
        &mut self,
        body_query: &Query<(&MultibodyRoot, Option<&ControlDescription>)>,
        multibody_select_event_reader: &mut EventReader<MultibodySelectionEvent>,
    ) {
            ...
            .show(ctx, |ui| {
                ui.vertical(|ui| {
                                ...
                                .show(ui, |ui| {
                                    ...
                                    for (joint_entity, joint_data) in joints.iter_mut() {
                                        ...
                                        match joint_data.joint_type {
                                            JointType::Revolute => {
                                                Self::revolute_slider(
                                                    ui,
                                                    joint_entity,
                                                    joint_data,
                                                    joint_motor_event_writer,
                                                );
                                            }
                                            JointType::Spherical => {
                    ...
                    if let Some(control_description) = control_description {
                        ui.heading("Control");
                        ui.label(control_description.clone());
                        ui.separator();
                    }
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button("Despawn").clicked() {
                            if let Some(entity) = multibody_root {
                                select_event_writer.send(SelectEvent::Deselect(*entity));
                                physic_request_event_writer
                                    .send(PhysicRequestEvent::DespawnBody(entity.to_bits()));
                            }
                        }
                    });
                });
    ...
    fn revolute_slider(
        ui: &mut Ui,
        joint_entity: &Entity,
        joint_data: &mut JointData,
        joint_motor_event_writer: &mut EventWriter<JointMotorEvent>,
    ) {
        if ui
            .add(
                egui::Slider::new(
                    &mut joint_data.val_axis_1,
                    Self::get_slider_range(joint_data.limits, &joint_data.joint_type),
                )
                .suffix("°")
                .step_by(0.1),
            )
    ...
    fn prismatic_slider(
            ...
            .changed()
        {
            ...
            joint_motor_event_writer.send(JointMotorEvent {
                entity: *joint_entity,
                command: MotorCommand::PositionPrismatic {
                    position: joint_data.val_axis_1,
                    stiffness: None,
                    damping: None,
                },
            });
    ...
    fn get_slider_range(
        limits: Option<Vec2>,
        joint_type: &JointType,
    ) -> RangeInclusive<rapier::Real> {
        match *joint_type {
            JointType::Revolute => match limits {
                Some(limits) => RangeInclusive::<rapier::Real>::new(
                    limits.x.to_degrees() as rapier::Real,
                    limits.y.to_degrees() as rapier::Real,
                ),
                None => RangeInclusive::<rapier::Real>::new(-180.0, 180.0),
            },
            JointType::Prismatic => match limits {
                Some(limits) => RangeInclusive::<rapier::Real>::new(
                    limits.x as rapier::Real,
                    limits.y as rapier::Real,
                ),
                None => RangeInclusive::<rapier::Real>::new(-1.0, 1.0),
            },
            _ => RangeInclusive::<rapier::Real>::new(-180.0, 180.0),
        }
    }
