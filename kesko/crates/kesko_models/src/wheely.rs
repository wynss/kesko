...
use kesko_physics::{
    joint::{
        fixed::FixedJoint, prismatic::PrismaticJoint, revolute::RevoluteJoint, JointMotorEvent,
        KeskoAxis, MotorCommand,
    },
    mass::Mass,
...
impl Wheely {
    ...
    fn handle_control_system(
        ...
        query: Query<
            (&MultibodyRoot, &WheelyController),
            (With<WheelyController>, With<MultibodyRoot>),
        >,
    ) {
            ...
            for event in wheely_event_reader.iter() {
                match event {
                    WheelyControlEvent::LeftWheelForward => {
                        ...
                        let action = MotorCommand::VelocityRevolute {
                            velocity: controller.wheel_velocity,
                            damping: None,
                        };
                    ...
                    WheelyControlEvent::ArmLink1Pos => {
                        ...
                        let action = MotorCommand::VelocityRevolute {
                            velocity: controller.arm_link1_velocity,
                            damping: None,
                        };
                        joint_event_writer.send(JointMotorEvent {
                            entity: *entity,
                    ...
                    WheelyControlEvent::ArmLink1Stop => {
                        let entity = root.child_map.get(ARM_LINK_1).expect("");
                        let action = MotorCommand::HoldPosition {
                            stiffness: Some(1.0),
                        };
                        joint_event_writer.send(JointMotorEvent {
                            entity: *entity,
                    ...
                    WheelyControlEvent::ArmLink2Pos => {
                        ...
                        let action = MotorCommand::VelocityPrismatic {
                            velocity: controller.arm_link2_velocity,
                            damping: None,
                        };
                        joint_event_writer.send(JointMotorEvent {
                            entity: *entity,
