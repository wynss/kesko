...
use kesko_physics::{event::collision::CollisionEvent, joint::JointMotorEvent};
...
pub fn log_joint_events(mut joint_event_reader: EventReader<JointMotorEvent>) {
    for joint_event in joint_event_reader.iter() {
        debug!("Joint event {:?}", joint_event);
    }
}

pub fn log_select_events(mut select_event_reader: EventReader<SelectEvent>) {
    for select_event in select_event_reader.iter() {
        debug!("Select event {:?}", select_event);
    }
}

pub fn log_interaction_events(mut interaction_event_reader: EventReader<InteractionEvent>) {
    for interaction_event in interaction_event_reader.iter() {
        debug!("Interaction event {:?}", interaction_event);
    }
}

pub fn log_collision_events(mut collision_event_reader: EventReader<CollisionEvent>) {
